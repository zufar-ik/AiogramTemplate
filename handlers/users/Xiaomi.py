...
from keyboards.default.model_redmi import redminot9, sheetX, modelListX, redmi_a, redmi_k
...
@dp.message_handler(text="Redmi/Redmi A", state=Phone.product)
async def key(message: types.Message):
    await message.answer("Выберите модель смартфона Redmi", reply_markup=redmi_a)
    await Phone.subproduct.set()


@dp.message_handler(text="Redmi K", state=Phone.product)
async def key(message: types.Message):
...
@dp.message_handler(text=modelListX, state=Phone.subproduct)
async def model_answer(message: types.Message, state: FSMContext):
    for i in modelListX:
        if message.text == i:
            n = modelListX.index(i)
            n += 1
            answersheet = (sheetX[f"B{n}:AV{n}"])
